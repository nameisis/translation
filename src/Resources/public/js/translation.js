    ...
    function ($scope, $location, $anchorScroll, sharedMessage, tableParamsManager, translationApiManager) {
        ...
        $scope.invalidateCache = function () {
            translationApiManager
                .invalidateCache()
                .success(function (responseData) {
                    sharedMessage.set('success', 'done', responseData.message);
                })
                .error(function () {
                    sharedMessage.set('danger', 'highlight_off', 'Error');
                })
            ;
        };
                    ...
                    if ((source == 'input' || source == 'textarea') && event.which == 27) { // escape key
                        ...
                        translationApiManager
                            ...
                            .success(function (data) {
                                ...
                                sharedMessage.set('success', 'done', translationCfg.label.updateSuccess.replace('%id%', data._key));
                            }).error(function () {
                            sharedMessage.set('danger', 'highlight_off', translationCfg.label.updateFail.replace('%id%', $scope.translation._key));
                        });
                    }
                };
                ...
                $scope.delete = function (column) {
                    ...
                    if (column.index == '_key') {
                        translationApiManager
                            ...
                            .success(function (data) {
                                sharedMessage.set('success', 'done', translationCfg.label.deleteSuccess.replace('%id%', data._key));
                                $scope.mode = null;
                                tableParamsManager.reloadTableData();
                            }).error(function () {
                            sharedMessage.set('danger', 'highlight_off', translationCfg.label.deleteFail.replace('%id%', $scope.translation._key));
                        });
                    } else {
                        translationApiManager
